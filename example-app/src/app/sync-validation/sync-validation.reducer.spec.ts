...
describe("sync validation", () => {
  describe(validateAndUpdateForm.name, () => {
    ...
    it("should validate that the password is required", () => {
      const state = validateAndUpdateForm(INITIAL_STATE);
      expect(
        state.controls.password.controls.password.errors.required
      ).toBeDefined();
    });
    ...
    describe("password", () => {
      const setPasswordValue = (v: string) =>
        updateGroup<FormValue>({
          password: updateGroup<FormValue["password"]>({
            password: setValue(v),
          }),
        });
      const setConfirmPasswordValue = (v: string) =>
        updateGroup<FormValue>({
          password: updateGroup<FormValue["password"]>({
            confirmPassword: setValue(v),
          }),
        });
      const setCreateAccountValue = (v: boolean) =>
        updateGroup<FormValue>({ createAccount: setValue(v) });
      ...
      it("should validate that the password must be at least 8 characters long", () => {
        ...
        expect(
          state.controls.password.controls.password.errors.minLength
        ).toBeDefined();
      ...
      it("should validate that confirmPassword must match password", () => {
        ...
        expect(state.controls.password.errors.passwordMatch).toEqual({
          password: passwordValue,
          confirmPassword: "",
        });
        const invalidConfirmPasswordValue = "abcefghj";
        state = validateAndUpdateForm(
          setConfirmPasswordValue(invalidConfirmPasswordValue)(state)
        );
        expect(state.controls.password.errors.passwordMatch).toBeDefined();
        expect(state.controls.password.errors.passwordMatch).toEqual({
          password: passwordValue,
          confirmPassword: invalidConfirmPasswordValue,
        });
        state = validateAndUpdateForm(
          setConfirmPasswordValue(passwordValue)(state)
        );
      ...
      it("should enable the password controls when createAccount is checked", () => {
        let state = validateAndUpdateForm(
          setCreateAccountValue(false)(INITIAL_STATE)
        );
        state = validateAndUpdateForm(
          setCreateAccountValue(true)(INITIAL_STATE)
        );
